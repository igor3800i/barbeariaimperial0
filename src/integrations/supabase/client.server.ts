...
// Server function — roda no servidor, bypassa RLS
const registerClient = createServerFn({ method: "POST" })
  .validator((d: { fullName: string; email: string; phone: string }) => d)
  .handler(async ({ data }) => {
...
function CadastroPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await registerClient({ data: { fullName: fullName.trim(), email, phone } });
      localStorage.setItem("imperial.client", JSON.stringify({ clientName: fullName.trim(), clientPhone: phone }));
